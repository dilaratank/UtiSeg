...
checkpoint_callback = ModelCheckpoint(monitor = "valid_f1_score", mode= 'max',
                                    ...
                                    filename= "model_best", 
                                    # 모델 저장될 때 모델명: {arch}_{encoder}" + "_{epoch:02d}_{valid_loss:.4f}.ckpt 
                                    
                                    dirpath ='./',
                                    # 모델 저장경로
                                    
                                    save_top_k = 1, # (최고로 좋은 모델 순으로) 모델 저장 개수
                                    
                                    # every_n_epochs = 1,
                                    # save_on_train_epoch_end = True,

                                    save_weights_only=True # 웨이트만 저장
                                    )
...
trainer.fit(
    model, 
    train_dataloaders = STILL_train_dataloader,
    val_dataloaders = STILL_val_dataloader)
